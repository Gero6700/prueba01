...
public class UpdatePeriodPricing : IUpdatePeriodPricing {
    ...
    public async Task<HttpResponseMessage> Execute(Conpreci conpreci) {
        if (conpreci.Cffec == 0) {
        ...
        return await availabilitySynchronizerApiClient.UpdatePeriodPricing(periodPricing);
    }
}
